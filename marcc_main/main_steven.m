...
point_mask(round(2*Nz/3),round(2*Nx/5)) = 1; 
point_mask(round(2*Nz/3),round(4*Nx/5)) = 1;
% medium matrix
...
for i = 1:N  %iterate through the elements for transmission and beamform
    ...
    %% Run the simulation
    filename = strcat(pwd, '/data',num2str(i));
    input_args = {'PlotSim', false, 'PMLSize', PML_Size, 'PMLInside', false, 'PlotFreq', 10};% 'RecordMovie', true};
    sensor_data = kspaceFirstOrder2D(kgrid, medium, source, sensor, input_args{:}, 'SaveToDisk', filename);
