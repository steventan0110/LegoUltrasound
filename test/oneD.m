...
Nx = 218;       % number of grid points in the x (row) direction
dx = 1e-3;   % grid point spacing in the x direction [m]
kgrid = kWaveGrid(Nx, dx);

% define the properties of the propagation medium
medium.sound_speed = 1420 * ones(Nx, 1);    % [m/s]
...
sensor.mask = [-10e-3, 10e-3, 20e-3];  % [mm]

% set the simulation time to capture the reflections
t_end = 2 * kgrid.x_size / max(medium.sound_speed(:));

% define the time array
kgrid.makeTime(medium.sound_speed, [], t_end);

% run the simulation
sensor_data = kspaceFirstOrder1D(kgrid, medium, source, sensor, 'PlotLayout', true, 'PlotSim', true);
